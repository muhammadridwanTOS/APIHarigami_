...
namespace API_Harigami.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class JundatePosController : Controller
    {
        public readonly IConfiguration _config;
        public string? constr;

        JundatePos db = new JundatePos();
        public JundatePosController(IConfiguration config)
        {
            _config = config;
        }
        ...
        public ActionResult InsertReplace([FromBody] dynamic prm)
        {
            ...
            try
            {
                ...
                List<dynamic> data = JsonConvert.DeserializeObject<List<dynamic>>(ObjectJSON["Data"]!.ToString())!;
                List<dynamic> datapos = JsonConvert.DeserializeObject<List<dynamic>>(ObjectJSON["DataPos"]!.ToString())!;
                resp = db.InsertReplace(constr, UserID, HrgmJundateIDArea, data, datapos);
                if (resp.ID == "0")
                {
                    return Ok(resp);
                }
