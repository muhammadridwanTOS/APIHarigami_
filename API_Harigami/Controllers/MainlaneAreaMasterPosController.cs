...
namespace API_Harigami.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class MainlaneAreaMasterPosController : Controller
    {
        public readonly IConfiguration _config;
        public string? constr;

        MainlaneAreaMasterPos db = new MainlaneAreaMasterPos();
        public MainlaneAreaMasterPosController(IConfiguration config)
        {
            _config = config;
        }
        ...
        [HttpPost]
        public ActionResult UPLOADREPLACE([FromBody] dynamic prm)
        {
            constr = _config.GetConnectionString("DefaultConnection");
            Response resp = new Response();
            ...
            try
            {
                ...
                List<dynamic> data = JsonConvert.DeserializeObject<List<dynamic>>(ObjectJSON["Data"]!.ToString())!;
                resp = db.UPLOADREPLACE(constr, UserID, data);
                if (resp.ID == "0")
                {
                    return Ok(resp);
                }
