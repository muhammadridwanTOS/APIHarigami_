...
namespace API_Harigami.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class JundatePartController : Controller
    {
        public readonly IConfiguration _config;
        public string? constr;

        JundatePart db = new JundatePart();
        public JundatePartController(IConfiguration config)
        {
            _config = config;
        }
        ...
        public ActionResult GetList([FromBody] dynamic prm)
        {
            ...
            try
            {
                ...
                string HrgmJundateIDArea = ObjectJSON["HrgmJundateIDArea"]!.ToString(); 
                ...
                resp = db.GetList(constr, ActionType, UserID, HrgmJundateIDArea, ModelCode, Ktsk, Sfx, ColorCode, data);
                if (resp.ID == "0")
                {
                    return Ok(resp);
                }
        ...
        public ActionResult InsertReplace([FromBody] dynamic prm)
        {
            ...
            try
            {
                ...
                string HrgmJundateIDArea = ObjectJSON["HrgmJundateIDArea"]!.ToString();
                List<dynamic> data = JsonConvert.DeserializeObject<List<dynamic>>(ObjectJSON["Data"]!.ToString())!;
                resp = db.InsertReplace(constr, UserID,  HrgmJundateIDArea,  ModelCode, Ktsk, Sfx, ColorCode, data);
                if (resp.ID == "0")
                {
                    return Ok(resp);
                }
