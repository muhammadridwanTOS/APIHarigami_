...
namespace API_Harigami.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class MainlaneAreaPartController : Controller
    {
        public readonly IConfiguration _config;
        public string? constr;

        MainlaneAreaPart db = new MainlaneAreaPart();
        public MainlaneAreaPartController(IConfiguration config)
        {
            _config = config;
        }
        ...
        public ActionResult GetList([FromBody] dynamic prm)
        {
            ...
            try
            {
                ...
                string HrgmMainIDArea = ObjectJSON["HrgmMainIDArea"]!.ToString(); 
                ...
                resp = db.GetList(constr, ActionType, UserID, HrgmMainIDArea, ModelCode, Ktsk, Sfx, ColorCode, data);
                if (resp.ID == "0")
                {
                    return Ok(resp);
                }
        ...
        public ActionResult InsertReplace([FromBody] dynamic prm)
        {
            ...
            try
            {
                ...
                string HrgmMainIDArea = ObjectJSON["HrgmMainIDArea"]!.ToString();
                List<dynamic> data = JsonConvert.DeserializeObject<List<dynamic>>(ObjectJSON["Data"]!.ToString())!;
                resp = db.InsertReplace(constr, UserID,  HrgmMainIDArea,  ModelCode, Ktsk, Sfx, ColorCode, data);
                if (resp.ID == "0")
                {
                    return Ok(resp);
                }
